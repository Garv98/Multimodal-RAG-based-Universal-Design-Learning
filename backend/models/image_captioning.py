...
def generate_alt_text(image_file, max_length=75, num_captions=1):
    ...
    try:
        ...
        # Performance note: previously we ran THREE Florence-2 inferences
        # per image (<CAPTION> + <DETAILED_CAPTION> + <OCR>) on CPU, which
        # took 2-4 minutes total. We now run only <DETAILED_CAPTION> — the
        # brief caption is derived by truncating it, and OCR is handled
        # upstream by TrOCR in `text_extraction.py`. This cuts the slow
        # path roughly 3x without losing any user-facing capability.
        ...
        try:
            ...
            # Fallback to brief caption only if detailed fails
            try:
                detailed_caption = generate_florence_caption(pil_image, "<CAPTION>")
                print(f"📝 Brief caption (fallback): {detailed_caption}")
            except Exception as e2:
                print(f"⚠️ Brief caption fallback also failed: {e2}")

        if detailed_caption:
            # Short alt-text = first sentence (or 140 chars max) of the
            # detailed caption — screen readers want a quick summary, not
            # a paragraph.
            brief_caption = _shorten_for_alt(detailed_caption)
            print(f"✅ Florence-2 caption ready (alt={len(brief_caption)} chars, detailed={len(detailed_caption)} chars)")

            enhanced_description = None
            if GROQ_API_KEY:
                print("🤖 Getting Groq enhanced description...")
                enhanced_description = get_groq_description(detailed_caption)
                if enhanced_description:
                    print(f"✅ Groq description: {enhanced_description[:100]}...")

            alt_text = _format_for_accessibility(brief_caption)
            ...
            result = {
                ...
                # OCR comes from TrOCR upstream; we no longer rerun it through
                # Florence-2 here. Frontend should read `extraction.text` for
                # the OCR'd content of an image.
                "ocr_text": None,
...
def _shorten_for_alt(text, max_chars=140):
    """Reduce a detailed caption to a screen-reader-friendly short alt-text.

    Prefers cutting at the first sentence boundary; falls back to a word
    ...
    # If there's a sentence boundary in the first max_chars, cut there.
    head = text[:max_chars]
    for marker in (". ", "! ", "? "):
    ...
    # Otherwise truncate at a word boundary and add an ellipsis.
